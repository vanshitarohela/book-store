...
insertProduct(book:Book){
  console.log(book)
  let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === book.ISBN)
  if(itemIndex===-1){
    this.cartItems.push(book)
    this.total = Number((this.total + book.price.value).toFixed(2));
  }
}

increaseOne(ISBN:number){
  let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBN)
  this.total = Number((this.total + (this.cartItems[itemIndex].price.value)).toFixed(2));
}

decreaseOne(ISBN:number){
  let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBN)
  this.total = Number((this.total - (this.cartItems[itemIndex].price.value)).toFixed(2));
}

removeItem(ISBNquantity:Removebook){
  let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBNquantity.ISBN)
  this.total = Number((this.total - ((ISBNquantity.quantity)*(this.cartItems[itemIndex].price.value))).toFixed(2));
